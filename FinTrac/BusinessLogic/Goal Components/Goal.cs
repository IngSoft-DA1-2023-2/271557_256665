...
public class Goal
{
    ...
    {
        if (MaxAmountToSpend < 0) throw new ExceptionValidateGoal("Error on max ammount to spent, cannot be negative");
    }

    private void ValidateTitle()
    {
        if (string.IsNullOrEmpty(Title)) throw new ExceptionValidateGoal("Error on goal tittle, it cannot be empty");
    }
