...
public class MonetaryAccount : Account
{
    ...
    public MonetaryAccount(string accountName, decimal amount, CurrencyEnum currencyType, DateTime creationDate) : base(
        accountName, currencyType, creationDate)
    {
        Amount = amount;
    ...
    {
        if (Amount < 0) throw new ExceptionValidateAccount("ERROR ON AMOUNT");
    }
    ...
    public override void UpdateAccountMoneyAfterAdd(Transaction transactionToBeAdded)
    {
        ...
        else if (IsIncome(transactionToBeAdded)) Amount = Amount + transactionToBeAdded.Amount;
    }

    public override void UpdateAccountAfterModify(Transaction transactionToBeAdded, decimal oldAmountOfTransaction)
    {
        if (IsIncome(transactionToBeAdded)) ModifyIncomeAmount(transactionToBeAdded, oldAmountOfTransaction);
        if (IsOutcome(transactionToBeAdded)) ModifyOutcomeAmount(transactionToBeAdded, oldAmountOfTransaction);
    }

    public override void UpdateAccountAfterDelete(Transaction transactionToBeDeleted)
    {
        if (IsIncome(transactionToBeDeleted)) Amount = Amount - transactionToBeDeleted.Amount;
        if (IsOutcome(transactionToBeDeleted)) Amount = Amount + transactionToBeDeleted.Amount;
    }
